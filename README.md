...
flowchart LR
    ...
    subgraph plant["machine_simulator（機台世界 / 假資料）"]
      MM["N×機台狀態機<br/>empty→check_in→working→check_out→done<br/>working→error→empty"]
    end
...
| 服務 | 職責 |
|---|---|
| **machine_simulator** | 每台機台一個狀態機，掌管 `empty→check_in→working→check_out→done` 與隨機 `error→downtime→empty`、即時計時。`autonomous` 自走（假資料）/ `driven` 由手臂指令驅動。 |
...
```
empty ──load──▶ check_in(Tray收合) ──▶ working ──▶ check_out(Tray吐出) ──▶ done ──unload──▶ empty
                            │
