...
3. **負載均衡**：多手臂拉動天然均衡；同時想拉隊頭時用決定性 tie-break（見 §5.4 可插拔規則）。
...
**輸入**：λ=進線速率(件/秒)=1/進線間隔、t_proc=機台作業時間、t_load=`arm_move_time_s`(line→機台)、t_unload=`arm_to_tray_time_s`(機台→盤)、A=機台妥善率。
