...
**FAIL/error 模型**：測試機的 error 來自**測試報告的 FAIL 列**（資料驅動，非隨機）。遇 FAIL 該筆照發（含解析後 path，走 `plant/machine/{id}/test`）→ 進 `error` 停 `fail_recovery_s` → 回 `working` 續測；整板 verdict 記為 FAIL 但**不中止、不報廢**。真機台的硬體故障仍走 `error → downtime → empty`（由真機台自行回報）。
