...
| 服務 | 職責 | 不負責 |
|---|---|---|
| **machine_simulator** | 每台機台一個測試機狀態機，掌管 empty/check_in/working/check_out/done/error 轉移、逐筆串流測項、FAIL→error 復原；對外發佈遙測與測項。**就是你猶豫的「處理器/機台模擬器」與「假資料產生器」的合體** | 不做排程、不知道產品從哪來 |
...
5. machine_simulator: M  empty→check_in(Tray收合)→working(作業時間計時)→check_out(Tray吐出)→done
       (working 逐筆串流測項；遇 FAIL → error(fail_recovery_s) → 回 working 續測，不報廢)
...
```
   arm放料     Tray收合      測項全測完     Tray吐出      arm取料
empty ──▶ check_in ──▶ working ──────▶ check_out ──▶ done ──▶ empty
                        │   ▲
                  FAIL  │   │ (fail_recovery_s 後回原處續測)
                        ▼   │
                       error ┘        （真機台硬體故障：error ──downtime──▶ empty）
```

| 狀態 | 意義 | 進入條件 | 離開條件 |
|---|---|---|---|
| `empty` | 無產品、可用 | 初始 / arm 取走 done / error downtime 結束 | arm 放料 → `check_in` |
| `check_in` | Tray 收合中 | arm 放料完成 | 經 `tray_check_in_time_s`(可為0) → `working` |
| `working` | 逐筆串流測項（每 `row_interval_s` 一筆；遇 FAIL 停 `fail_recovery_s` 再續，不中止） | check_in 完成 | 測項全測完 → `check_out` |
| `check_out` | Tray 吐出中 | working 完成 | 經 `tray_check_out_time_s`(可為0) → `done` |
| `done` | 完成、待取料（**會阻塞機台**） | check_out 完成 | arm 取料 → `empty` |
| `error` | **測試 FAIL 復原中**（測試機）；或真機台硬體故障 | working 中遇 FAIL（或真機台故障） | 測試機：`fail_recovery_s` 後 → `working` 續測（**不報廢**）；真機台：`downtime` → `empty` |

**FAIL/error 模型**：測試機的 error 來自**測試報告的 FAIL 列**（資料驅動，非隨機）。遇 FAIL 該筆照發（含解析後 `path` 與階層 `path_tree`，走 `plant/machine/{id}/test`）→ 進 `error` 停 `fail_recovery_s` → 回 `working` 續測；整板 verdict 記為 FAIL 但**不中止、不報廢**。真機台的硬體故障仍走 `error → downtime → empty`（由真機台自行回報）。
...
4. **工作中的 error = 復原中，不報廢**：測試機遇 FAIL 進 `error`，policy 視為機台**仍忙**（保留、繼續等 `done`），不 scrap、不中止；復原後機台回 `working`。（真機台若永久故障、`done` 永不來，未來以 recover 逾時保護，見 §12。）
...
需求手臂數  N_arm     ≥ ceil( λ · (t_load + t_unload) )  # 每件一上一下（測試機不報廢，皆完成）
```
...
| `plant/machine/{id}/test` | 上行 | machine_simulator | data_collector | 否 |
...
```json
{
  ...
  "process": { "machine_process_time_s": 20.0, "tray_check_in_time_s": 2.0, "tray_check_out_time_s": 2.0 },
  "arm": {
...
- **arm 時間為加法**：實際 = base + delta。`arm_move_time_s`/`arm_to_tray_time_s` 是每台共用的 base（夾取/接近），`per_machine` 的 `load_s`/`unload_s` 是該機台**額外加**的移動量；沒列到的機台 delta=0（就是 base）。
