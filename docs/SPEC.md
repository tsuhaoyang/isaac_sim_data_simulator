...
| 服務 | 職責 | 不負責 |
|---|---|---|
| **machine_simulator** | 每台機台一個狀態機，掌管 empty/check_in/working/check_out/done/error 轉移、即時執行時間、隨機故障 downtime；對外發佈遙測。**就是你猶豫的「處理器/機台模擬器」與「假資料產生器」的合體** | 不做排程、不知道產品從哪來 |
...
```
   arm放料      Tray收合        作業時間到      Tray吐出        arm取料
empty ──▶ check_in ──▶ working ──────▶ check_out ──▶ done ──▶ empty
                          │
                          │ hazard(隨機) → 台上產品報廢
                          ▼
                        error ──(downtime 結束)──▶ empty
```
...
| `working` | 加工中（回報 elapsed/remaining） | check_in 完成 | 作業時間到 → `check_out`；或 hazard 觸發 → `error` |
